...
class GeminiService
{
    ...
    // =========================================================================
    // [MODIFIKASI] Inject TavilyService via constructor
    // =========================================================================
    private TavilyService $tavily;

    public function __construct(TavilyService $tavily)
    {
        $this->tavily          = $tavily;
    ...
    // =========================================================================
    // Core Gemini caller — tidak ada perubahan
    // =========================================================================
    ...
    // =========================================================================
    // JSON helpers — tidak ada perubahan
    // =========================================================================
    ...
    // =========================================================================
    // generateRoadmap — prompt tidak berubah
    // =========================================================================
...
ATURAN RESOURCE:
- Kosongkan array resources di setiap topik: \"resources\": []
- Sistem akan mengisi resource secara otomatis dari sumber eksternal setelah roadmap dibuat.
  ...
  \"phases\": [
    {
      ...
      \"topics\": [
        {
          \"topic_title\": \"string\",
          \"description\": \"string (WAJIB detail 2-3 paragraf dalam format Markdown)\",
          \"order\": integer,
          \"resources\": []
        }
      ]
    ...
    // =========================================================================
    // [MODIFIKASI] handleRoadmapGeneration — tambah enrichment setelah AI selesai
    // =========================================================================
    ...
    public function handleRoadmapGeneration(array $validated, Skill $skill): Roadmap
    {
        ...
        // -----------------------------------------------------------------
        // [BARU] Enrich resources setiap topik dengan hasil pencarian Tavily.
        // Dilakukan SEBELUM transaksi DB agar tidak memperlambat DB transaction.
        // Jika Tavily gagal, proses tetap lanjut (resources kosong).
        // -----------------------------------------------------------------
        if (!empty($aiResult['phases'])) {
            Log::info('TavilyService: Mulai enrichment resources roadmap', [
                'skill' => $skill->name,
            ]);

            $aiResult['phases'] = $this->tavily->enrichRoadmapResources(
                phases:    $aiResult['phases'],
                skillName: $skill->name
            );

            Log::info('TavilyService: Enrichment selesai', [
                'skill' => $skill->name,
            ]);
        }
    ...
    // =========================================================================
    // generateFeedback — tidak ada perubahan
    // =========================================================================
...
Format JSON yang harus dikembalikan:
{
  ...
  \"perlu_penyesuaian_jadwal\": boolean
    ...
    // =========================================================================
    // evaluateRoadmap — tidak ada perubahan
    // =========================================================================
...
2. JANGAN mengubah judul topik lama kecuali diminta spesifik.
...
Format JSON:
{
  ...
  \"phases\": [
    ...
    // =========================================================================
    // [MODIFIKASI] handleRoadmapEvaluation — enrich resources untuk topik baru
    // =========================================================================
    ...
    public function handleRoadmapEvaluation(Roadmap $roadmap, array $validated): Roadmap
    {
        ...
        // -----------------------------------------------------------------
        // [BARU] Enrich HANYA topik baru (yang resources-nya masih kosong).
        // Topik lama akan diisi dari $oldTopicsData (database), bukan Tavily.
        // -----------------------------------------------------------------
        if (!empty($aiResult['phases'])) {
            $protectedTitles = collect($currentPhases)
                ->flatMap(fn($p) => collect($p['topics'])->pluck('topic_title'))
                ->map(fn($t) => trim($t))
                ->toArray();
            ...
            Log::info('TavilyService: Enrichment selesai (evaluation)', [
                'skill'           => $roadmap->skill->name,
                'protected_count' => count($protectedTitles),
            ]);
        }
        ...
        return DB::transaction(function () use ($oldTopicsData, $roadmap, $validated, $aiResult) {
            ...
            foreach ($aiResult['phases'] as $phaseData) {
                ...
                foreach ($phaseData['topics'] as $topicData) {
                    ...
                    // Prioritas resource: topik lama (DB) → Tavily (new) → kosong
                    $resources = (!empty($oldTopic['resources']))
                        ? $oldTopic['resources']
                        : ($topicData['resources'] ?? []);
